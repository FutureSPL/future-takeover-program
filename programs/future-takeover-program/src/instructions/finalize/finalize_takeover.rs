...
use crate::{
    errors::TakeoverError, state::{AdminProfile, Takeover, SuccessfulTakeover, FailedTakeover, Level::*, Phase::* }
};
...
impl<'info> FinalizeTakeover<'info> {
    ...
    pub fn migrate_to_failed(&mut self) -> Result<()> {
        ...
        // Transform to SuccessfullTakeover
        let successful_takeover = FailedTakeover {
    ...
    match ctx.accounts.takeover.inflation_amount.level {
        Low => {
            if success_amount < ctx.accounts.takeover.presale_claimed {
                ctx.accounts.migrate_to_succesfull()?;
            } else {
